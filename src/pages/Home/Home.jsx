  ...
  return (
    <>
      ...
      <section className="flex items-center justify-center gap-4 py-7 font-gillSans">
        <button className="bg-yellow-400 text-yellow-100 rounded-xl p-2 focus:bg-yellow-900 focus:text-yellow-400 " onClick={() => setFilterOption("asc")}>
          Date Ascending
        </button>
          ...
          <section className="flex flex-wrap items-center justify-center gap-10 font-gillSans">
            {movies.map((movie, index) =>
              movie.title.toLocaleLowerCase().includes(input.toLocaleLowerCase()) ? (
                <div key={index} className="bg-yellow-900 w-52 h-72 p-3 flex flex-col text-center rounded-xl">
